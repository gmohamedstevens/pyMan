...
# Get dictionary from word list URL
WORD_SITE = "https://www.mit.edu/~ecprice/wordlist.10000"
RESPONSE = requests.get(WORD_SITE)
DICTIONARY = RESPONSE.content.splitlines() # Gets list of words from URL of bytes type
MAX_TRIES = 7
...
def get_word():
    return random.choice(DICTIONARY).decode('UTF-8') # Decode byte type to string type
...
def valid_guess_made():
    if not len(letter_guess) == 1: # Guess must be only one character
        ...
        if not letter_guess.isalpha(): # Guess must be in the alphabet
            ...
            return False
        if letter_guess in guessed_letters: # Cannot guess the same letter twice
    ...
    while tries_left > 0 and not player_won: # While there are tries left and the player hasn't won, keep playing
        ...
        if valid_guess_made():
            ...
            if letter_guess in secret_word:
                ...
                if not "_" in display_word: # If there are no more hidden letters, the player has won
